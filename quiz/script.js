var submit_button = document.getElementById('submitBtn');

function checkAnswers() {
    var q1_answer = document.querySelector('input[name="q1"]:checked')?.value;
    var q2_answer = document.querySelector('input[name="q2"]:checked')?.value;
    var q3_answer = document.querySelector('input[name="q3"]:checked')?.value;
    var q4_answer = document.querySelector('input[name="q4"]:checked')?.value;
    var q5_answer = document.querySelector('input[name="q5"]:checked')?.value;

    if (q1_answer == "a") {
        alert("Correct!");
    } else {
        alert("Incorrect. The correct answer is a) Lebron James");
    }
    if (q2_answer == "b") {
        alert("Correct!");
    } else {
        alert("Incorrect. The correct answer is b) Milwaukee Bucks");
    }
    if (q3_answer == "c") {
        alert("Correct!");
    } else {
        alert("Incorrect. The correct answer is c) Shai Gilgeous-Alexander");
    }
    if (q4_answer == "d") {
        alert("Correct!");
    } else {
        alert("Incorrect. The correct answer is d) Jerry West");
    }
    if (q5_answer == "a") {
        alert("Correct!");
    } else {
        alert("Incorrect. The correct answer is a) Bill Russell");
    }
}

submit_button.addEventListener("click", checkAnswers);
